    ...
    <div class="textbox_nop">
        <div class="userpage_info">
            <div class="userpage_info_left">
                <div class="cb"><h1>Ваши рецепты</h1></div>
                <?php Pjax::begin([
                    'linkSelector' => '.pjax'
                ]) ?>
                        ...
                        <div class="inputbox_2_col">
                            ...
                            <div class="inputbox_label_right">
                                <select name="occasion-selector" class="select_base" data-for-link="occasion-selector-link">
                                    <option value="<?= Url::to(['/users/recipes', 'id' => $user->id]) ?>">Все рецепты</option>
                ...
                <?php if (!count($provider->models)): ?>
                    <div class="no-counts">
                        <?php if (Yii::$app->user->id === $user->id): ?>
                            <?= Yii::$app->settings->get('emptyBlockForRecipes') ?>
                        <?php else: ?>
                            Пользователь не добавил ещё ни одного рецепта
                        <?php endif; ?>
                    </div>
                <?php else: ?>
